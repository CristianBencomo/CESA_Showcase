...
connection = obd.OBD() #Connect to obd port
...
def loop():
    while(True):
        ...
        #Read obd responses
        res_speed = connection.query(speed) # send the command, and parse the response
        res_rpm = connection.query(rpm)
        res_throttle = connection.query(throttle)
        res_cooltemp = connection.query(cool_tmp)
        res_fuel = connection.query(fuel)
        res_ambtmp = connection.query(amb_tmp)
        res_distance = connection.query(distance)
        ...
        "ambient temp" : amb_tmp, "fuel" : res_fuel, "distance" : res_distance}
